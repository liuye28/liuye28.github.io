/**
 * 纯 JavaScript 实现的标准 MD5 哈希计算算法
 *
 * 背景：浏览器原生 Web Crypto API (crypto.subtle) 出于安全弃用考虑并不原生支持 MD5 摘要算法。
 * 本工具提供零依赖、开箱即用的离线纯 JS MD5 计算，保证在纯本地前端环境下依然能进行 MD5 签名与校验。
 *
 * @param {string} string 待计算哈希的原始字符串
 * @returns {string} 32 位小写 MD5 哈希十六进制字符串
 */
